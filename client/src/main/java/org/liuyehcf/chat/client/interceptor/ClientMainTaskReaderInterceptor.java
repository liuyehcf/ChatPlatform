    ...
    private void processSystemMessage(ClientMainConnection connection, Message message) {
        if (message.getHeader().getParam1().equals(LOGIN_REPLY)) {
            ...
            if (message.getHeader().getParam3().equals(PERMIT)) {
                //获取主界面
                MainWindow mainWindow = connection.getBindMainWindow();
        ...
        } else if (message.getHeader().getParam1().equals(OPEN_SESSION_WINDOW)) {
            //开启一条从fromUserName到toUserName的会话窗口
            String fromUserName = message.getHeader().getParam2();
            String toUserName = message.getHeader().getParam3();
            ClientUtils.ASSERT(fromUserName.equals(connection.getConnectionDescription().getSource()));

            MainWindow mainWindow = clientConnectionDispatcher.getMainConnectionMap().get(fromUserName).getBindMainWindow();
